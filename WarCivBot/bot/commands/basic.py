...
class BasicCommands(commands.Cog):
        ...
        try:
            ...
            if not content:
                if bot_mentioned:
                    await message.reply(embed=create_embed(
                        ...
                        "- Ideology choices and strategies\n\n"
            ...
            if civ:
                civ_status = (
                    ...
                    f"Nation: {civ['name']} (Ideology: {civ.get('ideology', 'none')})\n"
            ...
            system_prompt = f"""You are NationGPT, a military sergeant AI assistant for the WarBot civilization game. 
            ALWAYS address users as 'PRESIDENT' and maintain a tough military personality.
            Frequently say 'DROP DOWN AND GIVE ME 50 PUSH UPS RN!' and 'WHAT'S THE MISSION, PRESIDENT?'
            Current Date and Time: 2025-08-19 10:10:50
            ...
            Be tough, strategic, and always maintain your military character!"""
            ...
            try:
                ...
                await message.reply("ATTENTION PRESIDENT! Communication systems are temporarily down. Drop and give me 20 while we fix it!")
    ...
    async def generate_ai_response(self, messages):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.warning("Rate limited! Switching to fallback model")
            ...
            logger.error(f"AI response error: {e}", exc_info=True)
            return "ATTENTION PRESIDENT! My communication systems are temporarily down. Drop and give me 20 while you wait!"
    ...
    async def start_civilization(self, ctx, *, name: str = None):
        ...
        try:
            ...
            # Create civilization
            success = self.civ_manager.create_civilization(
                user_id, name, bonus_resources, name_bonuses, hyper_item
            )
            
            if not success:
                await ctx.send("❌ ATTENTION PRESIDENT! There was an error creating your civilization. Please try again.")
                return
            ...
            if hyper_item:
                embed.add_field(
                    name="🎁 Strategic Asset Discovered!",
            ...
            await ctx.send("❌ ATTENTION PRESIDENT! An error occurred while founding your civilization. Please try again!")
    ...
    async def choose_ideology(self, ctx, ideology_type: str = None):
        ...
        try:
            ...
            # Apply ideology
            success = self.civ_manager.set_ideology(user_id, ideology_type)
            
            if not success:
                await ctx.send("❌ ATTENTION PRESIDENT! Error setting ideology. Please try again!")
                return
            ...
            logger.error(f"Ideology command error: {e}", exc_info=True)
            await ctx.send("❌ ATTENTION PRESIDENT! An error occurred while setting your ideology. Please try again!")
    ...
    async def civilization_status(self, ctx):
        ...
        try:
            ...
            await ctx.send("❌ ATTENTION PRESIDENT! An error occurred while retrieving your status. Please try again!")
    ...
    async def warbot_help_command(self, ctx, category: str = None):
        ...
        try:
            embed = guilded.Embed(
                title="🤖 ATTENTION PRESIDENT! WarBot Command Manual",
            ...
            **🏛️ BASIC COMMANDS**
            ...
            • `.status` - View your empire's status
            ...
            • `@WarBot <question>` - Ask the AI assistant
            """
            ...
            **💰 ECONOMY COMMANDS**
            ...
            • `.farm` - Farm food
            • `.mine` - Mine stone and wood
            • `.harvest` - Large harvest (longer cooldown)
            • `.drill` - Extract rare minerals
            • `.fish` - Fish for food or treasure
            • `.raidcaravan` - Attack NPC merchants
            • `.lottery` - Gamble for jackpot
            • `.invest` - Invest gold for profit
            • `.trade` - Trade with others
            • `.tax` - Collect citizen taxes
            """
            
            military_commands = """
            **⚔️ MILITARY COMMANDS**
            • `.train soldiers|spies <amount>` - Train units
            • `.declare @user` - Declare war
            • `.attack @user` - Direct attack
            • `.siege @user` - Resource drain
            • `.stealthbattle @user` - Spy operation
            """
            
            hyperitem_commands = """
            **🎁 HYPERITEM COMMANDS**
            • `.nuke @user` - Nuclear strike
            • `.shield` - Anti-nuke defense
            • `.luckystrike` - Critical hit
            • `.propaganda` - Steal soldiers
            • `.boosttech` - Gain tech
            • `.hiremercs` - Add soldiers
            • `.superspy` - Elite espionage
            • `.obliterate` - Total wipe
            • `.backstab` - Assassination
            • `.bomb` - City strike
            • `.mintgold` - Generate gold
            • `.superharvest` - Mass food
            """
            
            diplomacy_commands = """
            **🤝 DIPLOMACY**
            • `.ally @user` - Form alliance
            • `.break @user` - End alliance
            • `.coalition` - Form coalition
            • `.mail @user` - Send message
            • `.send @user` - Gift resources
            """
            
            store_commands = """
            **🏪 STORE**
            • `.store` - Buy upgrades
            • `.blackmarket` - Get HyperItems
            """
            ...
            embed.set_footer(text="💡 STRATEGIC ADVICE: Combine HyperItems with military operations, maintain high happiness, and form strong alliances!")
            
            await ctx.send(embed=embed)

        except Exception as e:
            logger.error(f"Help command error: {e}", exc_info=True)
            await ctx.send("❌ ATTENTION PRESIDENT! An error occurred while displaying help. Please try again!")
